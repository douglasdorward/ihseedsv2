...
import { getCategories, getProductBySlug, getProducts, getRedirect, type CatalogueCategory, type CatalogueProduct } from "../../../lib/catalogue";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = details.seoTitle?.trim() || `${product.name} | IH Seeds`;
  const description = details.seoDescription?.trim() || details.blurb?.trim() || product.note;
  ...
  return {
    ...
    openGraph: { type: "website", url: canonical, title: details.socialTitle?.trim() || title, description: details.socialDescription?.trim() || description, images: [{ url: image, alt: product.name }] },
  };
}
...
export default async function ProductPage({ params }: Props) {
  ...
  const defaultLine = product.saleLines?.find((line) => line.isDefault) ?? product.saleLines?.[0];
  ...
  const alsoPopular = allProducts
    ...
    .slice(0, 3);
  const price = priceValue(defaultLine?.priceDisplay || product.price);
  const productJsonLd = {
    ...
    description: details.seoDescription || details.blurb || product.note,
    image,
      ...
      <section>
        <div className="product-detail-grid">
          <article className="product-main-col">
            ...
            <section style={{ display: "grid", gap: 16 }}><h2 style={{ color: "var(--green)", fontSize: 28 }}>How it&apos;s sold</h2><div className="product-table-wrap availability-table"><table style={{ width: "100%", borderCollapse: "collapse" }}><thead><tr><th>Form</th><th>Pack</th><th>Status</th></tr></thead><tbody>{product.saleLines?.map((line) => <tr key={line.stockCode}><td>{line.seedForm || "Bare"}</td><td>{line.packKg ? `${line.packKg} ${line.packUnit}` : product.packSize}</td><td><StatusPill status={({ "Good stock": "in-stock", "Low stock": "low", "Very low": "very-low", Unavailable: "unavailable" }[line.availability ?? "Unavailable"] ?? product.status)} /></td></tr>)}</tbody></table></div></section>
            </div>
          </article>
          <aside className="product-sidebar-col">
            <div className="product-sidebar-card mobile-order-2">
              ...
              <div className="pricing-section"><StatusPill status={product.status} /><div className="pricing-details"><strong className="pricing-amount">{defaultLine?.priceDisplay || product.price || "Contact for pricing"}</strong><span className="pricing-unit">Available in {defaultLine?.packKg ? `${defaultLine.packKg} ${defaultLine.packUnit}` : product.packSize}</span></div><Link href="/contact" className="button button-primary" style={{ width: "100%", textAlign: "center" }}>Ask about an order</Link><small className="pricing-disclaimer">We supply through rural resellers across Western Australia.</small></div>
            </div>
