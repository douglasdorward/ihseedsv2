  ...
  if (applied.rows[0].count === 0) {
    const latestSchema = await client.query(`
      SELECT
        ...
        (
          to_regclass('ih_admin_users') IS NOT NULL
          AND to_regclass('ih_admin_pending_approvals') IS NOT NULL
          AND to_regclass('ih_admin_revocations') IS NOT NULL
          AND to_regclass('ih_admin_access_audit') IS NOT NULL
        ) AS admin_ready,
        (
          to_regclass('ih_media_assets') IS NOT NULL
          AND to_regclass('ih_media_references') IS NOT NULL
        ) AS media_ready,
        (
          to_regclass('ih_site_settings') IS NOT NULL
        ) AS site_settings_ready,
        (
          to_regclass('ih_articles') IS NOT NULL
        ) AS articles_ready,
        (
          to_regclass('ih_reseller_brands') IS NOT NULL
          AND to_regclass('ih_reseller_outlets') IS NOT NULL
        ) AS resellers_ready,
        (
          EXISTS (
            SELECT 1 FROM information_schema.columns
            WHERE table_schema = current_schema() AND table_name = 'ih_articles'
              AND column_name = 'scheduled_publish_at'
          )
        ) AS articles_schedule_ready,
        (
          EXISTS (
            SELECT 1 FROM information_schema.columns
            WHERE table_schema = current_schema() AND table_name = 'ih_reseller_outlets'
              AND column_name = 'latitude'
          )
        ) AS reseller_coordinates_ready
    `);
    if (latestSchema.rows[0].ready) {
      const baselineBefore = !latestSchema.rows[0].admin_ready
        ? "0017_"
        : !latestSchema.rows[0].media_ready
          ? "0019_"
          : !latestSchema.rows[0].site_settings_ready
            ? "0024_"
            : !latestSchema.rows[0].articles_ready
              ? "0025_"
              : !latestSchema.rows[0].resellers_ready
                ? "0026_"
                : !latestSchema.rows[0].articles_schedule_ready
                  ? "0029_"
                  : !latestSchema.rows[0].reseller_coordinates_ready
                    ? "0035_"
                    : null;
      const baselineMigrations = baselineBefore
        ? migrations.filter((migration) => migration.file < baselineBefore)
        : migrations;
      ...
      console.log(`Baselined ${baselineMigrations.length} migrations for an existing journal-less schema`);
    }
  }
