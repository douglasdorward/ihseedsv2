...
try {
  ...
  if (existing.rows[0].count > 0) {
    ...
    const rootByName = new Map(
      seed.categories
        .filter((category) => category.sourceParentId === null)
        .map((category) => [category.name, category.slug]),
    );
    const fallbackCategorySlug = (name) => name.toLowerCase().normalize("NFKD").replace(/[\u0300-\u036f]/g, "")
      .replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "") || "catalogue";
    const legacyWebsitePath = (value) => {
    ...
    for (const product of seed.products) {
      const fromPath = legacyWebsitePath(product.websiteUrlLegacy);
      if (!fromPath) continue;
      const toPath = `/products/${rootByName.get(product.category) || fallbackCategorySlug(product.category)}/${product.slug}`;
    ...
    const souwest = seed.products.find((product) => product.slug === "souwest-pasture-mix");
    if (souwest) {
      const toPath = `/products/${rootByName.get(souwest.category) || fallbackCategorySlug(souwest.category)}/${souwest.slug}`;
      await client.query(
        `INSERT INTO ih_redirects (from_path, to_path)
         VALUES ($1, $2)
         ON CONFLICT (from_path) DO UPDATE SET to_path = EXCLUDED.to_path, updated_at = now()`,
        ["/product/souwest-pasture-mix", toPath],
      );
    }
